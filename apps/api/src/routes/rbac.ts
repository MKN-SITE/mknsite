...
import { authorizeAdmin, authorizeSuperadmin } from "../guards/admin.guard";
import {
...
const detail = (operationId: string, summary: string, mutation = false) => ({
  ...
  description: mutation
    ? "Operasi RBAC sensitif; memerlukan izin admin.security.manage dan dicatat dalam audit log."
...
export const rbacRoutes = new Elysia({ prefix: "/admin/rbac" })
  ...
  .post("/roles", async ({ request, body, status }) => {
    ...
    if (!auth.success) return status(auth.failure.status, auth.failure.error);
    return sendResult(status, await rbacService.createRole(body, auth.admin.id), 201);
  ...
  .patch("/roles/:id", async ({ request, params, body, status }) => {
    const auth = await authorizeSuperadmin(request);
    if (!auth.success) return status(auth.failure.status, auth.failure.error);
    return sendResult(status, await rbacService.updateRole(Number(params.id), body, auth.admin.id));
  ...
  .delete("/roles/:id", async ({ request, params, status }) => {
    const auth = await authorizeSuperadmin(request);
    if (!auth.success) return status(auth.failure.status, auth.failure.error);
    return sendResult(status, await rbacService.deleteRole(Number(params.id), auth.admin.id));
  ...
  .post("/permissions", async ({ request, body, status }) => {
    const auth = await authorizeSuperadmin(request);
    if (!auth.success) return status(auth.failure.status, auth.failure.error);
    return sendResult(status, await rbacService.createPermission(body, auth.admin.id), 201);
  ...
  .patch("/permissions/:id", async ({ request, params, body, status }) => {
    const auth = await authorizeSuperadmin(request);
    if (!auth.success) return status(auth.failure.status, auth.failure.error);
    return sendResult(status, await rbacService.updatePermission(Number(params.id), body, auth.admin.id));
  ...
  .delete("/permissions/:id", async ({ request, params, status }) => {
    const auth = await authorizeSuperadmin(request);
    if (!auth.success) return status(auth.failure.status, auth.failure.error);
    return sendResult(status, await rbacService.deletePermission(Number(params.id), auth.admin.id));
