...
export function generateOncallFormNumber(id: number): string {
  return `OC-${new Date().getFullYear()}-${String(id).padStart(5, "0")}`;
}
...
export function validateStatusTransition(
  currentStatus: OncallJobStatus,
  targetStatus: OncallJobStatus,
  isPicUser: boolean,
  isSpvUser: boolean
): void {
  // Matriks transisi status yang valid:
  const allowedTransitions: Record<OncallJobStatus, OncallJobStatus[]> = {
  ...
  if (currentStatus === "draft" && targetStatus === "technician_signing") {
    ...
    if (!isPicUser && !isSpvUser) {
      throw new OpsTelcoFormError(403, "Hanya PIC, pembuat form, atau Supervisor yang dapat mengajukan Job Oncall.");
    }
...
export async function getFullJobDetail(formId: number, profile: AuthenticatedProfile) {
  const [form] = await db
    .select()
    .from(opsTelcoForms)
  ...
  if (!isSpv && !isUserParticipant && !isCreator) {
    throw new OpsTelcoFormError(403, "Anda tidak memiliki akses ke Job Oncall ini.");
  }
  ...
  return {
    ...form,
    status: form.status as OncallJobStatus,
    data: parsedData,
    participants,
    signatures,
    approvalHistory,
...
export async function logOncallAudit(
  actorId: number,
  action: string,
  resourceId: string,
  ipAddress?: string
