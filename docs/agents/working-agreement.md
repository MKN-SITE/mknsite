...
- Untuk setiap penambahan fitur modul bisnis atau antarmuka portal baru, agen WAJIB mendaftarkan izin (`permissions`), peran (`roles`), dan kartu menu di `apps/api/src/db/seed.ts` dalam PR yang sama (Standar Otomasi Rilis Modul Portal). Dilarang mengandalkan input manual di production.
- Jika melakukan `git revert` pada PR yang mengubah skema database atau menambah data (seed, menu, role), agen WAJIB membersihkan data orphan di database secara terpisah. `git revert` hanya mengembalikan kode, BUKAN data di database. Lihat [database-ops.md](database-ops.md) untuk prosedur lengkap.
- Sebelum deploy atau revert, periksa tabel `__drizzle_migrations` untuk membandingkan state migrasi antara lokal dan production. Lihat [database-ops.md](database-ops.md#migration-history-__drizzle_migrations).
